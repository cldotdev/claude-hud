 ...
 *   ? <path>                   → untracked
 */
export function parseGitStatusV2(output: string): GitStatus | null {
  let branch = '';
  let oid = '';
  let ahead = 0;
  let behind = 0;
...
export async function getGitStatus(cwd?: string): Promise<GitStatus | null> {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(
      'git',
      ['--no-optional-locks', 'status', '-b', '--porcelain=v2'],
      { cwd, timeout: 1000, encoding: 'utf8' }
    );
...
async function getGitStatusLegacy(cwd: string): Promise<GitStatus | null> {
  try {
    // Get branch name
